...
add_variable_cb(GtkWidget *widget, MathVariablePopup *popup)
{
    ...
    name = ctk_entry_get_text(GTK_ENTRY(popup->priv->variable_name_entry));
    if (name[0] == '\0')
        return;
...
make_variable_entry(MathVariablePopup *popup, const gchar *name, const MPNumber *value, gboolean writable)
{
    ...
    if (writable)
    {
        ...
        image = ctk_image_new_from_icon_name("document-save", GTK_ICON_SIZE_BUTTON);
        ...
        button = ctk_button_new();
        g_object_set_data(G_OBJECT(button), "variable_name", g_strdup(name));
        image = ctk_image_new_from_icon_name("edit-delete", GTK_ICON_SIZE_BUTTON);
        ctk_container_add(GTK_CONTAINER(button), image);
        ctk_box_pack_start(GTK_BOX(hbox), button, FALSE, TRUE, 0);
                                 ...
                                 GParamSpec   *pspec)
{
    ...
    case PROP_EQUATION:
        ...
        for (i = 0; names[i]; i++) {
            ...
            ctk_box_pack_start(GTK_BOX(self->priv->vbox), entry, FALSE, TRUE, 0);
        }
        g_strfreev(names);

        entry = ctk_box_new(GTK_ORIENTATION_HORIZONTAL, 6);
        ctk_widget_show(entry);
        ...
        ctk_box_pack_start(GTK_BOX(entry), self->priv->variable_name_entry, TRUE, TRUE, 0);
        ...
        ctk_container_add(GTK_CONTAINER(self->priv->add_variable_button), image);
        ctk_box_pack_start(GTK_BOX(entry), self->priv->add_variable_button, FALSE, TRUE, 0);
        ctk_widget_show(image);
        ctk_widget_show(self->priv->add_variable_button);
        ctk_box_pack_end(GTK_BOX(self->priv->vbox), entry, FALSE, TRUE, 0);
...
math_variable_popup_init(MathVariablePopup *popup)
{
    ...
    ctk_window_set_skip_taskbar_hint(GTK_WINDOW(popup), TRUE);

    ctk_container_set_border_width(GTK_CONTAINER(popup), 6);
    ...
    popup->priv->vbox = ctk_box_new(GTK_ORIENTATION_VERTICAL, 6);
    ctk_box_set_homogeneous(GTK_BOX(popup->priv->vbox), TRUE);
    ctk_container_add(GTK_CONTAINER(popup), popup->priv->vbox);
